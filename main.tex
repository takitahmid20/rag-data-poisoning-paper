...
The 100 percentage-point reduction is structurally linked to the gate's conservative source rule: the known untrusted file is rejected before indexing. It is not an estimate of recall against an unknown attacker. A malicious file placed under an apparently trusted path, an instruction paraphrased without the current regular-expression phrases, or an encoded Unicode payload could evade the current implementation. Conversely, a legitimate emergency policy update could be quarantined and produce a false positive. The correct interpretation is therefore control-point validation plus a reproducible baseline, not a complete security certification.
